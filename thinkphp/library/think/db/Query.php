...
class Query
{
    ...
    public function page($page, $listRows = null)
    {
        ...
        $this->options['worker'] = [intval($page), intval($listRows)];
        return $this;
    }
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        $page  = isset($config['worker']) ? (int) $config['worker'] : call_user_func([
            $class,
            'getCurrentPage',
        ], $config['var_page']);
        ...
        if (!isset($total) && !$simple) {
            ...
            unset($this->options['order'], $this->options['limit'], $this->options['worker'], $this->options['field']);
    ...
    protected function parseExpress()
    {
        ...
        if (isset($options['worker'])) {
            // 根据页数计算limit
            list($page, $listRows) = $options['worker'];
            $page                  = $page > 0 ? $page : 1;
